...
gsap.utils.toArray(".floatfast").forEach((star) => {
  tweenPropertyFast(star, "scale", 0.7, 1.3);
  tweenPropertyFast(star, "x", -300, 300);
  tweenPropertyFast(star, "y", -300, 300);
});
gsap.utils.toArray(".floatslow").forEach((star) => {
  tweenPropertySlow(star, "scale", 0.8, 1.2);
  tweenPropertySlow(star, "x", -60, 60);
  tweenPropertySlow(star, "y", -60, 60);
});
gsap.utils.toArray(".floatveryslow").forEach((star) => {
  tweenPropertyFast(star, "x", -25, 25);
  tweenPropertyFast(star, "y", -25, 25);
});
gsap.utils.toArray(".rotatefast").forEach((star) => {
  tweenPropertyFast(star, "rotation", -160, 160);
});
gsap.utils.toArray(".rotateslow").forEach((star) => {
  tweenPropertySlow(star, "rotation", -15, 15);
});
gsap.utils.toArray(".impulse").forEach((star) => {
  tweenPropertySlow(star, "scale", 1, 2);
  tweenPropertySlow(star, "x", -120, 120);
  tweenPropertySlow(star, "y", -120, 120);
});
gsap.utils.toArray(".impulse2").forEach((star) => {
  tweenPropertySuper(star, "scale", 0.1, 2);
  tweenPropertySuper(star, "x", -200, 200);
  tweenPropertySuper(star, "y", -300, 200);
});

var superprop = true;

function tweenPropertyFast(target, prop, min, max) {
  var randomDur = gsap.utils.random(4, 6, 0.2, true);
  ...
  gsap.to(target, {
    ...
    delay: randomDelay(),
    ease: "none",
    onComplete: tweenPropertyFast,
    onCompleteParams: [target, prop, min, max]
  });
}
function tweenPropertySlow(target, prop, min, max) {
  var randomDur = gsap.utils.random(6, 12, 0.2, true);
  var randomDelay = gsap.utils.random(0.6, 2, 0.2, true);
  gsap.to(target, {
    [prop]: gsap.utils.random(min, max),
    duration: randomDur(),
    delay: randomDelay(),
    ease: "none",
    onComplete: tweenPropertySlow,
    onCompleteParams: [target, prop, min, max]
  });
}
function tweenPropertySuper(target, prop, min, max) {
  var randomDur = gsap.utils.random(0.2, 0.4, 0.2, true);
  var randomDelay = gsap.utils.random(0.1, 2, 0.2, true);
  var randomprop = gsap.utils.random(min, max);

  if (prop == "scale") {
    if (randomprop < 0.4 ) superprop = true;
    if (randomprop > 1.6 ) superprop = false;
    if (superprop == true  ) randomprop = gsap.utils.random(0.1, 0.6);
    if (superprop == false ) randomprop = gsap.utils.random(0.4, 1.2);
  }

  if (superprop == true ) randomDur = gsap.utils.random(1, 1.5, 0.2, true);
